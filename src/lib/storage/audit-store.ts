...
const storeDir = path.join(process.cwd(), ".nexusguard");
...
export async function appendAuditEntry(userId: string, entry: AuditEntry) {
  const db = await runWithDatabase("appendAuditEntry", async (pool) => {
    await pool.query(
      `
        INSERT INTO nexusguard_audit_entries (id, user_id, timestamp, payload)
...
export async function resetAuditState(userId: string) {
  const db = await runWithDatabase("resetAuditState", async (pool) => {
    await pool.query("DELETE FROM nexusguard_audit_entries WHERE user_id = $1", [userId]);
    await pool.query(
      `
        INSERT INTO nexusguard_usage (user_id, spent_xlm, tool_calls, last_updated)
