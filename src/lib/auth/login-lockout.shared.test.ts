...
describe("login lockout shared state", () => {
  ...
  it("keeps lockout state across module reloads", async () => {
    ...
    firstModule.registerLoginFailure("operator@nexusguard.local", "10.9.0.1");
    firstModule.registerLoginFailure("operator@nexusguard.local", "10.9.0.1");

    expect(firstModule.isLoginLocked("operator@nexusguard.local", "10.9.0.1").locked).toBe(true);

    vi.resetModules();

    const secondModule = await import("@/lib/auth/login-lockout");
    const lockState = secondModule.isLoginLocked("operator@nexusguard.local", "10.9.0.1");
