...
function operatorCookie() {
  process.env.NEXUSGUARD_AUTH_SECRET = "metrics-test-secret";
  const token = createSessionToken({
    email: "ops@nexusguard.local",
    role: "operator",
...
describe("/api/metrics route", () => {
  ...
  it("returns JSON shape with expected fields", async () => {
    ...
    expect(body.service).toBe("nexusguard");
  ...
  it("returns Prometheus text format with required metrics", async () => {
    ...
    expect(text).toContain("# HELP nexusguard_requests_total Total API requests by route/method");
    expect(text).toContain("# TYPE nexusguard_requests_total counter");
    expect(text).toContain("nexusguard_requests_total{");

    expect(text).toContain("# HELP nexusguard_request_errors_total Total API errors by route/method");
    expect(text).toContain("# TYPE nexusguard_request_errors_total counter");
    expect(text).toContain("nexusguard_request_errors_total{");

    expect(text).toContain("# HELP nexusguard_request_duration_ms_p95 P95 request duration in milliseconds");
    expect(text).toContain("# TYPE nexusguard_request_duration_ms_p95 gauge");
    expect(text).toContain("nexusguard_request_duration_ms_p95{");

    const lines = text.trim().split("\n");
    for (const line of lines) {
      if (line.startsWith("nexusguard_")) {
