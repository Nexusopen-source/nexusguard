...
function resolveRoleByCredentials(email: string, password: string): AuthRole | null {
  ...
  const operatorEmail = process.env.NEXUSGUARD_OPERATOR_EMAIL?.trim().toLowerCase();
  const operatorPassword = process.env.NEXUSGUARD_OPERATOR_PASSWORD;

  if (operatorEmail && operatorPassword && normalizedEmail === operatorEmail && password === operatorPassword) {
    return "operator";
  }

  const viewerEmail = process.env.NEXUSGUARD_VIEWER_EMAIL?.trim().toLowerCase();
  const viewerPassword = process.env.NEXUSGUARD_VIEWER_PASSWORD;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requiredMfaCode = process.env.NEXUSGUARD_MFA_CODE?.trim();
