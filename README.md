...
NexusGuard is a **policy-controlled payment safety layer for autonomous agents on Stellar**.
...
## 1) What NexusGuard Does

NexusGuard acts like an execution firewall:
...
- Cookie: `nexusguard_session`
- HMAC-signed token (`NEXUSGUARD_AUTH_SECRET` required)
- Roles: `operator`, `viewer`

Wallet login is role-mapped via allowlisted public keys:
- `NEXUSGUARD_OPERATOR_WALLETS` (comma-separated `G...` keys)
- `NEXUSGUARD_VIEWER_WALLETS` (comma-separated `G...` keys)
...
- If `NEXUSGUARD_SHARED_STATE_PATH` is set, lockout and rate-limit state is persisted to a shared file.
...
NexusGuard does **not** store private keys.
...
If DB is unavailable (or not configured), the system falls back to `.nexusguard/*.json` stores.
